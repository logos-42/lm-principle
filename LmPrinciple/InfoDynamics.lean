...
/-- **T7 FFN 可坍缩**: 前馈网络 (无回馈, y = σ(W·x) 无恒等项)
    ...
    注意模态: 恒等/门控回馈是防坍缩的**充分保证** (T6 给距离下界);
    无回馈路径**存在**坍缩实现 (本定理), 即无回馈不保证不坍缩——
    但回馈不是"必要条件" (恒等 FFN σ=id, W=1 不坍缩)。 -/
...
/-- **T10 综合假设验证**: "自由能最小化依赖特定信息结构"——
    把已证定理组装: 残差防坍缩 (ArchCompare) + LSTM 保留
    (ArchCompare) + MoE 凸组合误差界 (Fractal) + 分形参数分配
    (Fractal)。
    结论: 依赖的信息结构 = ①恒等/门控回馈 (防坍缩+保留) ②凸组合
    (误差有界) ③前缀参数集中 (分形分配)。(交叉熵下界 T8 是独立
    定理, 不在本合取内。) -/
