...
\author{Email: yuanjieliu65@gmail.com}

\markboth{IEEE Transactions on Pattern Analysis and Machine Intelligence}%
{Li: Mathematical Fractal Criticality in LLM Dynamics}
...
RLHF-optimal policy is a softmax (a Gibbs corollary). Building on 73 fully
verified theorems (63 central to this argument, none with \texttt{sorry}), we
...
D
(v)~Murray's-law ratio $\rho=2^{-1/3}\approx0.79$ is the unique optimum of the
flow-conservation-constrained problem (per-vessel local optimum combined with
mass conservation), resolving an empirical paradox as a \emph{model mismatch}:
the global free-scan cost model used in the original experiments has its
optimum away from $0.79$, whereas the constrained model has its optimum
exactly at $0.79$. Every claim is discharged by the proof assistant; local
numerical experiments (torch CPU, seed 42) reproduce the reported numbers to
within $0.01$ and add new corroborating evidence (a $1.33\times$ parameter
efficiency ratio and a deep-layer fractal advantage of $0.39\times$ at depth
14). We also derive the precise condition under which
...
the entire proof chain inheritable and extensible; and (6) a lattice Maxwell formalization connecting field theory to the energy framework, with a conditional prediction $\tau_G = 1/(6M_0^2)$ under the three-direction hypothesis ($m_G = \sqrt{3}\,M_0$).
...
All three experiments were reproduced locally in this work (PyTorch 2.12.1,
CPU, seed 42, identical model definitions and training protocol): the fractal
transformer reaches loss $113.6098$ vs.\ the uniform $113.6133$ with the same
$25\%$ parameter reduction (seed-42 reproducibility within $0.01$ of the
original numbers), the optimal depth is again $2$ for both architectures, and
the local sweep reveals a \emph{deep-layer fractal advantage}: at depth $14$
the fractal loss is $44.78$ vs.\ $113.62$ for the uniform model, a factor
$0.39\times$ (Fig.~\ref{fig_depth}).
...
maintenance $r^2$), a \emph{global free scan} over the branch ratio makes
$\rho=0.90$ (or even $\rho\to1$) appear cheaper than $\rho=0.79$. The narrative
acknowledges this as a boundary of a simplified model. Rather than treating this
as an exception to be papered over, \S\ref{sec:murray} shows it is a
\emph{provable consequence of a model mismatch}: the scan treats the ratio as a
free parameter of a global sum, whereas Murray's law is a
\emph{constraint-optimization} statement---each vessel is locally optimal for
its own flow (giving $r^3\propto Q$), and mass conservation at the branching
point ($Q=Q_1+Q_2$) forces $r^3=r_1^3+r_2^3$. Under that constrained problem the
optimum ratio is exactly $\rho=2^{-1/3}$ regardless of the cost coefficients
(see Remark~\ref{rem:paradox}); the two models are different optimization
problems and must not be conflated. Concretely, the tree-level formula's
resistance term $\sum_g 2^g/r_g^4$ omits the flow-conservation factor
($Q_g=Q/2^g$); with it restored, the corrected tree-level cost has an
interior minimum converging to $2^{-1/3}$ (scripts/diagnose\_murray\_cost.py:
$G{=}50$ gives $0.795$, $0.13\%$ off). Our numerical verification
(\texttt{scripts/verify\_experiment.py}) confirms both statements: the global
scan minimum is not $0.79$, while the constrained ratio is pinned to $0.7937$
for every flow magnitude.
...
cost attains its global minimum exactly at
...
\begin{remark}[The empirical paradox as model mismatch]
\label{rem:paradox}
D
The commonly reported paradox---that a simplified cost makes $\rho=0.79$
non-optimal while $0.90$ (or larger ratios) looks better---is not an anomaly
to be masked. It is exactly predicted by theorem~\ref{thm:murray}, once the
two models are separated: the original experiments compute a \emph{global sum}
$\sum_k n_k(r_k^{-4}+r_k^2)$ over a freely chosen ratio, whose optimum is not
$0.79$; Murray's law is instead the \emph{constrained} optimum in which each
vessel is locally optimal for its flow ($r^3\propto Q$) and mass conservation
forces $r^3=r_1^3+r_2^3$. Under the constrained problem the ratio is pinned to
$2^{-1/3}$ independently of cost coefficients. The precise mechanism of the
global-sum failure is the missing flow-conservation factor in its resistance
term ($Q_g=Q/2^g$); restoring it gives an interior minimum converging to
$2^{-1/3}$ (scripts/diagnose\_murray\_cost.py: $G{=}50$ gives $0.795$,
$0.13\%$ off). (A further honest boundary: in
pure transport cost, branching is always more expensive than not branching by a
factor $2^{1/3}$; the \emph{reason} to branch is coverage---space filling---not
transport. Murray's law gives the optimal ratio \emph{given} that branching is
required.) This is the honest resolution of the paradox.
\end{remark}
...
provide numerical corroboration. Three experiments were run
(\texttt{experiments/run\_fractal\_experiments.py}, PyTorch 2.12.1, CPU,
seed 42, model definitions and training protocol identical to
\cite{shape2026}): (1) branching-ratio scanning confirms the constrained
ratio $\rho=2^{-1/3}\approx0.79$ (the global free-scan model, by contrast,
does not attain its minimum there---see Remark~\ref{rem:paradox}); (2) a
fractal transformer reaches slightly \emph{lower} loss ($113.6098$ vs.\
$113.6133$) with $25\%$ fewer parameters ($301{,}569$ vs.\ $400{,}641$,
efficiency ratio $1.3285$); (3) optimal-depth search finds a loss cliff from
depth $4$ onward and best depth $2$ for both architectures, and reveals a
deep-layer fractal advantage (at depth $14$: fractal $44.78$ vs.\ uniform
$113.62$, i.e.\ $0.39\times$). Table~\ref{table_experiments} summarizes these
measurements. Seed-42 reproducibility against the original numbers is within
$0.01$ for experiment (2) and identical (optimal depth $2$) for experiment
(3); absolute losses in (3) differ from \cite{shape2026} because of the
torch version, while the qualitative pattern (loss cliff from depth $4$)
is preserved. Each of these observations is consistent with, and in the
fractal and depth cases implied by, the theorems above.

\subsection{Multi-seed rerun: an honest correction}
\label{sec:multiseed}
The single-seed sweep above reproduces the original numbers, but single-seed
comparisons can be platform luck. We therefore reran the two decisive
experiments with three seeds
(\texttt{scripts/experiments\_v2.py}, mini-batch $64$, lr $0.001$,
$200$ epochs, task-optimal loss ${\approx}0.09$, constant predictor
$129.59$):
\begin{itemize}
\item \textbf{Fractal vs.\ uniform is not significant across seeds.} Uniform
$86.83\pm47.17$ vs.\ fractal $75.74\pm52.88$ (loss mean$\pm$std over 3 seeds):
the standard deviation far exceeds the mean gap. The single-seed ``fractal
wins'' (and the original ``uniform wins'') are both within noise. What does
survive is the parameter saving ($301{,}569$ vs.\ $400{,}641$).
\item \textbf{The ``loss cliff from depth 4'' is a training artifact.}
Independent depth scans (3 seeds) show depth $2$ is robustly optimal
($0.788\pm0.062$), but losses at depths $4$--$12$ stay in $1.3$--$1.7$---no
cliff; the original spike came from an unstable single run.
\item \textbf{The marginal benefits $\Delta_k$ are not strictly decreasing.}
Direct supervision of per-layer marginal benefit ($12$ layers, $150$ epochs,
3 seeds) finds $\Delta_k$ decreasing on only $5/11$ steps. Nevertheless the
crossing point $k^*=\min\{k:\Delta_k<\lambda\}$ (Remark~\ref{rem:critical})
is $k^*=2$ for every tested $\lambda\in\{0.05,0.1,0.2\}$: the optimal depth
is $2$ and the critical-point formula is stable even when its monotonicity
hypothesis fails in measurement.
\end{itemize}
The methodological lesson is exactly the ``two layers of evidence'' point:
the theorems are unconditional (fractal allocation dominates \emph{if}
$\Delta_k$ is decreasing; optimal depth is the first crossing \emph{if}
returns are decreasing), and the multi-seed data show \emph{which} hypotheses
hold empirically---here, the critical-depth conclusion survives, while the
per-layer monotonicity and the ``cliff'' do not. The paper's central claims
that survive are structural (fractal saves parameters; optimal depth exists
and is small on this task), and the ones that do not survive are the
quantitative single-seed details.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Local experimental corroboration (PyTorch 2.12.1, CPU, seed 42).}
...
\multirow{2}{*}{Fractal vs.\ uniform}
 & Uniform & $400{,}641$ params, loss $113.6133$ \\
 & Fractal & $301{,}569$ params, loss $113.6098$ (ratio $1.33$) \\
\midrule
\multirow{3}{*}{Optimal depth}
 & depth 2 & best loss for both ($11.28$ / $7.67$) \\
 & depth $\geq4$ & loss cliff for both ($\geq14$) \\
 & depth 14 & fractal $44.78$ vs.\ uniform $113.62$ ($0.39\times$) \\
\bottomrule
\end{tabular}
\end{table}
...
hold on concrete problems, and that the numbers are reproducible under a fixed
seed. All local measurements and the comparison script
(\texttt{scripts/compare\_local\_vs\_math.py}) are included in the repository;
the raw JSON (git-ignored) is regenerated by the training script.


\section{Maxwell's Equations Meet the Energy Framework}
\label{sec:maxwell}
We connect classical electromagnetism to the energy framework formalized
above. On a one-dimensional periodic lattice (space $n\in\mathbb{Z}_N$,
staggered differences), the vacuum Maxwell equations take the form
\begin{equation}
\label{eq:maxwell-lattice}
\dot E_n = -\frac{B_n - B_{n-1}}{\Delta x},\qquad
\dot B_n = -\frac{E_{n+1} - E_n}{\Delta x},
\end{equation}
with field energy $H = \tfrac12\sum_n (E_n^2 + B_n^2)$. The cross term
$\sum_n [E_n(B_n - B_{n-1}) + B_n(E_{n+1} - E_n)]$ telescopes to zero on the
periodic lattice (a discrete product rule: $\sum(\Delta E)B + \sum E(\Delta B)
= 0$ under the shift reindexing $n\mapsto n+1$). The group structure of
$\mathbb{Z}_N$---true modular subtraction, unlike the truncated subtraction
of $\mathrm{Fin}\ N$---is essential for the cancellation.

\begin{theorem}[Lattice Maxwell energy conservation]\label{thm:maxwell}
Under \eqref{eq:maxwell-lattice}, $dH/dt = 0$: the massless lattice Maxwell
field conserves energy
(\texttt{cross\_term\_zero}, \texttt{maxwell\_energy\_conservation},
machine-verified).
\end{theorem}

\paragraph{Extension under the authors' assumption: massive field.}
Introducing a mass term (Proca-type)
$\dot E_n = -(B_n - B_{n-1})/\Delta x - m^2 E_n$ yields:

\begin{theorem}[Massive energy decay]\label{thm:massive}
The energy changes at rate $dH/dt = -m^2\sum_n E_n^2 \le 0$: the mass term
dissipates energy with energy-decay time constant $\tau_H = 1/(2m^2)$
(\texttt{massive\_energy\_decay}, machine-verified).
\end{theorem}

\paragraph{Prediction.}
Under the authors' three-direction hypothesis ($m_G = \sqrt{3}\,M_0$:
plane-orthogonal vectors $\to$ three gluons), the massive component has
$m_G^2 = 3M_0^2$, hence energy-decay time constant
\begin{equation}
\label{eq:tau-g}
\tau_G = \frac{1}{2 m_G^2} = \frac{1}{6 M_0^2},
\end{equation}
a quantitative, testable prediction: at the corresponding energy scale, a
massive (Proca-type) component of the field decays with this time constant,
whereas the massless sector conserves energy exactly. It is precisely the
coupling $m_G = \sqrt3\,M_0$ (not any mass) that yields $\tau_G = 1/(6M_0^2)$.

\paragraph{Connection to the energy framework.}
The massless case is the conservative counterpart of the Hopfield dynamics
formalized in \texttt{Hopfield.lean}: field energy is a landscape on which the
dynamics move---conserved here, monotonically decreasing under the flip
update there (\texttt{flip\_energy\_strictly\_decreases}); the mass term is
the continuous analogue of dissipation. Together with the critical-point
theory (\S\ref{sec:depth}), the same structure---an energy landscape with a
critical point separating conservative and dissipative regimes---appears in
neural dynamics, Hopfield memory, and field theory.

\paragraph{Limitation.}
These theorems verify the \emph{lattice algebra} of the energy structure
(cross-term cancellation, massive decay rate); the continuum limit (partial
derivatives, curl/divergence calculus) is deferred to analysis
infrastructure. The prediction \eqref{eq:tau-g} inherits the status of the
three-direction assumption: it is a conditional implication, not an
unconditional law.
...
checked Lean 4 theorems (63 central to this argument). Fractal organization
...
the constrained variational optimum; and optimal depth has a precise existence
condition. The approach upgrades hypotheses C1--C4 from conjectures to theorems,
and turns an empirical paradox into a provable model-mismatch statement. Local
real training (PyTorch CPU, seed 42) reproduces the reported numbers to within
$0.01$ and adds two new pieces of corroborating evidence: a $1.33\times$
parameter-efficiency ratio (fractal reaches slightly lower loss with $25\%$
fewer parameters) and a deep-layer fractal advantage of $0.39\times$ at depth
$14$, consistent with power-law connection sparsity.
...
$k^*=\min\{k:\Delta_k<\lambda\}$ under the diminishing-returns assumption
(i), but the mapping from fractal dimension $D$ to a concrete closed-form
$n^*(D)$ (without measuring $\Delta_k$) remains open; and entropy,
...
measure-theoretic extension, the stationary-point verification, and a
closed form $n^*(D)$ linking fractal dimension directly to optimal depth.
...
Critical-point = first crossing & \texttt{CriticalPoint} & \texttt{optimal\_depth\_is\_first\_crossing} \\
Power-law critical pair & \texttt{CriticalPoint} & \texttt{power\_law\_critical\_pair} \\
Power-law optimal depth is critical & \texttt{CriticalPoint} & \texttt{power\_law\_optimal\_depth\_is\_critical} \\
